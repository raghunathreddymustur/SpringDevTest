package com.raghu.springboot.test.application;

import com.raghu.springboot.test.configuration.SampleDataConfiguration;
import com.raghu.springboot.test.ds.BookingResult;
...
import com.raghu.springboot.test.service.GuestSharableDataService;
...
import static com.raghu.springboot.test.ds.BookingResult.BookingState.NO_ROOM_AVAILABLE;
import static com.raghu.springboot.test.ds.BookingResult.BookingState.ROOM_BOOKED;
...
public class ApplicationServiceIntegrationTest {
    ...
    private void assetReservationAtDate(BookingResult bookingResult, LocalDate expectedReservationDate) {
        LocalDate reservationDate = bookingResult.getReservation().get().getReservationDate();
    ...
    private void assertGuestRegisteredCorrectly(BookingResult bookingResult, String firstName, String lastName) {
        Guest guest = bookingResult.getReservation().get().getGuest();
    ...
    private void assertRoomReservedFromAvailableRooms(BookingResult bookingResult, Set<Room> availableRooms) {
        Room reservedRoom = bookingResult.getReservation().get().getRoom();
    ...
    private Reservation getReservationFromRepository(BookingResult bookingResult) {
        return reservationRepository.findById(bookingResult.getReservation().get().getId()).get();
    }

    private void assertReservationSaved(BookingResult bookingResult, Reservation savedReservation) {
        assertEquals(bookingResult.getReservation().get(), savedReservation);
    }

    private void assertReservationForSpecificRoom(BookingResult bookingResult, String roomName) {
        assertEquals(roomName, bookingResult.getReservation().get().getRoom().getName());
    }
}